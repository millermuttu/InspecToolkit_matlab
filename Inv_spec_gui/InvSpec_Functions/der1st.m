function X_1st_der = der1st(X, order, window)
% Summary: Function to compute the 1st derivative of data
...
%
%% Calculate 1st derivative (copy of online script)
% Transpose X
X=X';
...
B = savitzkyGolay(pp,order,1,pp,W);
...
% Preallocate output
X_1st_der = zeros(size(X));

% Compute the transient on
X_1st_der(1:(window+1)/2-1,:) = fliplr(B(:,(window-1)/2+2:end)).'*flipud(X(1:window,:));

% Compute the steady state output
ytemp = filter(B(:,(window-1)./2+1),1,X);
X_1st_der((window+1)/2:end-(window+1)/2+1,:) = ytemp(window:end,:);

% Compute the transient off
X_1st_der(end-(window+1)/2+2:end,:) = fliplr(B(:,1:(window-1)/2)).'*flipud(X(end-(window-1):end,:));

% Convert Y to the original shape of X
if isempty(DIM)
	X_1st_der = shiftdim(X_1st_der, -nshifts);
else
	X_1st_der = ipermute(X_1st_der,perm);
