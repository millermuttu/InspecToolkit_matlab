function [Spectra,listboxItems,val] = excel_read_fn_uni(hObject,handles)
val=handles.val;
...
elseif size(temp,2)<2 || size(temp,1)<5
    mymsgbox('Too few cells selected. You must select at least two columns and five rows.','a|e')
    return
end
% listboxlistboxItems{1}=handles.Demo_1;
% handles.data_index{0}=handles.Demo_1;
if size(temp,2)>1
    WL = temp(:,1); % Wavelengths vector
    data = temp(:,2:end);
    for k = 1:1:size(data,2);
        Spectra{k+val} = [WL,data(:,k)];
        listboxItems{k+val} = strcat('Spectra',num2str(k+val));
    end
end
val=k+val;
end
